...
<#
.SYNOPSIS
Runs one of the five established Hantek DSO5102P device actions.
.DESCRIPTION
...
param(
    ...
    [ValidateSet('identify', 'echo', 'screenshot', 'acquisition-start', 'acquisition-stop')]
    [string]$Action,
    [string]$PythonPath,
...
-LogDir           Acquisition action logs, otherwise artifacts/logs in this repo.
...
if (-not $Action) {
    throw 'Specify identify, echo, screenshot, acquisition-start, or acquisition-stop. Use -Help for examples.'
}
if ($PSBoundParameters.ContainsKey('OutputDir') -and $Action -ne 'screenshot') {
    throw '-OutputDir applies only to screenshot.'
}
if ($PSBoundParameters.ContainsKey('LogDir') -and $Action -notin @('acquisition-start', 'acquisition-stop')) {
    throw '-LogDir applies only to acquisition-start or acquisition-stop.'
}
...
if ([string]::IsNullOrWhiteSpace($scopeGuid) -or
...
$clientArguments = @($runtime.Prefix) + @($scopeClient, '--guid', $parsedScopeGuid.ToString(), $Action)
